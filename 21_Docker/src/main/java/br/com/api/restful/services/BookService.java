...
public class BookService {
	...
	@Autowired
	private PagedResourcesAssembler<BookVO> assembler; //montadora
	...
	public PagedModel<EntityModel<BookVO>> findAllWithPagedModel(Pageable pageable) {

		logger.info("Finding all bookse with pageable model!");
		
		var booksPage = repository.findAll(pageable);
		
		var bookVOsPage = booksPage.map(b -> DozerMapper.parseObject(b, BookVO.class)); //converter person entity para VO

		bookVOsPage.map(b -> b.add(linkTo(methodOn(BookController.class).findById(b.getKey())).withSelfRel())); //add links HATEOAS
